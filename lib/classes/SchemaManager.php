...
class SchemaManager
{
    ...
    public function update(Command $cmd, $dbNamespace = null)
    {
        ...
        if ($versions === 0 || $versions === 1) {
            ...
            if ($isSchemaChanged) {
                ...
                $sql = $this->diff($schemaFrom, $schemaTo, $cmd);
            } else {
                _writeln();
        ...
        if (count($sql['up']) == 0 && count($dropConstraintSql) == 0 && count($createConstraintSql) == 0) {
            return false;
        }
        ...
        $dump = '--'.PHP_EOL
            ...
            .'-- '.date('r').PHP_EOL
            .'--'.PHP_EOL.PHP_EOL
            .implode(PHP_EOL.PHP_EOL, $sql['up']);
        ...
        file_put_contents($versionDir._DS_.$dbVersion.$this->sqlExtension, $dump);
     ...
     * @return array
     */
    public function diff($schemaFrom, $schemaTo, Command $cmd)
    {
