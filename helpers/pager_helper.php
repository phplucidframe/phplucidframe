...
function pager_calculateOffset($pageNum, $itemsPerPage){
...
function pager_pageNum($rowCount, $itemPerPage, $pageNumLimit, $pageNum=1){
	$navArray = array();
	$navArray['url'] 		= '';
	$navArray['imagePath'] 	= '';
	$navArray['ajax'] 		= false;		
	$navArray['thisPage'] 	= $pageNum; // pageNum is current page
	
	$maxPage = ceil($rowCount/$itemPerPage);		
    if($pageNum <= $pageNumLimit){
	  $startPage = 1;
	}else{
		//$startPage = $pageNum - 3;//eg. this form =>2345
	  $startPage = (floor(($pageNum-1)/$pageNumLimit)*$pageNumLimit)+1;//this form =>5678
	} 
		  
    $j = 0;	
	$k = 0;
	$navArray['beforePages'] = array();
	$navArray['afterPages'] = array();
	for($pageCount=0, $i = $startPage ; $i<=$maxPage; $i++){
					
		if($i < $pageNum){
			$navArray['beforePages'][$j] = $i;
			$j++;
		}
							
		if($i > $pageNum){
			$navArray['afterPages'][$k] = $i;
			$k++;
		}
		
		$pageCount ++;	  				
		if($pageCount == $pageNumLimit)//display page number only.
		break;
	...
	if ($pageNum > 1){
	   ...
	   $navArray['firstPageNo'] = 1;
	   $navArray['firstPageEnable'] = 1;
	 }else{
	   $navArray['firstPageEnable'] = 0;
 	 }
				
	//Previous Page
	if ($pageNum > 1){
	
	   $navArray['prePageNo'] = $pageNum-1;
	   $navArray['prePageEnable'] = 1;
		  
	} else{
		$navArray['prePageEnable'] = 0;
	}
	
	//Next page no.
	if ($pageNum<$maxPage)
	{
	   $navArray['nextPageNo'] = $pageNum + 1;
	   $navArray['nextPageEnable'] = 1;

	   $navArray['lastPageNo'] = $maxPage;
	   $navArray['lastPageEnable'] = 1;
	   
	}else{
	   $navArray['nextPageEnable'] = 0;
	   $navArray['lastPageEnable'] = 0;
	}
	//Display multi page or not
	if(($maxPage <= 1) || ($pageNum > $maxPage))
		$navArray['multiPageEnable'] = 0;
	else 
		$navArray['multiPageEnable'] = 1;	
	
	//if page count is less than page num limit, fill page num till page num limit
	if($maxPage > $pageNumLimit){
		$allPagesCount = count($navArray['beforePages']) + count($navArray['afterPages']) + 1;
		if($allPagesCount < $pageNumLimit){
			$page = $pageNum - 1;
			$filledPageCount = $pageNumLimit - $allPagesCount;
			if(isset($navArray['beforePages'])) $filledPageCount += count($navArray['beforePages']);	
			$x = 0;
			while($filledPageCount != $x){
				$filledPages[] = $page;
				$page--;
			...
			$navArray['beforePages'] = array();
			sort($filledPages);
			$navArray['beforePages'] = $filledPages;		
		}
	}
 	return $navArray;				 
}

if(!function_exists('pager_display')){
/**
 * Display the pager
 * @param $navBar	(array) The array returned from the function pager_pageNum()
 */
	function pager_display($navBar){
		$url 		= ($navBar['url']) ? $navBar['url'] : NULL;
		$ajax 		= $navBar['ajax'];
		$imagePath 	= isset($navBar['imagePath']) ? $navBar['imagePath'] : '';
				
		if($navBar['multiPageEnable']){
			extract($navBar);
			?>
			<table class="pagerTable" border="0" cellpadding="0" cellspacing="0"><tr> 
			<?php
			# first
			if($firstPageEnable){
			?>
				<td class="first-enabled">
				<?php if($ajax){ ?>                
					<a href="<?php echo _url($url); ?>" rel="<?php echo $firstPageNo; ?>">
				<?php }else{ ?>
					<a href="<?php echo _url($url, array('page' => $firstPageNo)); ?>">
...
/* Ajax respond for pager */
function pager_respond($id){
	$get = _get($_GET);
	if(isset($get['_'])) unset($get['_']);
	if(isset($get['route'])) unset($get['route']);
	if(sizeof($get)){
		echo '[script]';
		echo 'Page.queryStr["_'.$id.'"] = '.json_encode($get).';';
	}
}